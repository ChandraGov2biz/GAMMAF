...
- Removed metrics functionality to prevent errors
...
BEDROCK_INFERENCE_PROFILE = os.environ.get(
    ...
    'arn:aws:bedrock:us-east-2:203918860835:inference-profile/us.anthropic.claude-3-5-haiku-20241022-v1:0'
)
...
AGENT_CAPABILITIES = {
    "knowledge_base": {
        ...
        "inference_profile_arn": BEDROCK_INFERENCE_PROFILE
    },
    "payment_inquiry": {
...
def validate_environment():
    ...
    if not BEDROCK_MODEL_ID:
        raise ValueError("BEDROCK_MODEL_ID is missing or empty")
    if not BEDROCK_REGION:
        raise ValueError("BEDROCK_REGION is missing or empty")
    if not BEDROCK_INFERENCE_PROFILE:
        raise ValueError("BEDROCK_INFERENCE_PROFILE is missing or empty")
    
    # Log that we're using defaults if environment variables aren't set
    if 'DYNAMODB_TABLE' not in os.environ:
        logger.info("Using default DYNAMODB_TABLE: LiaAgentConversations")
    if 'BEDROCK_MODEL_ID' not in os.environ:
        logger.info("Using default BEDROCK_MODEL_ID: anthropic.claude-3-haiku-20240307-v1:0")
    if 'BEDROCK_REGION' not in os.environ:
        logger.info("Using default BEDROCK_REGION: us-east-2")
    if 'BEDROCK_INFERENCE_PROFILE' not in os.environ:
        logger.info(f"Using default BEDROCK_INFERENCE_PROFILE: {BEDROCK_INFERENCE_PROFILE}")
...
def store_conversation(conversation_id, conversation_data):
    ...
    logger.info(f"STORE_CONVERSATION: Storing data for conversation_id: {conversation_id}, Data: {json.dumps(conversation_data, cls=DecimalEncoder)}")
...
def update_conversation(conversation_id, updates):
    ...
    logger.info(f"UPDATE_CONVERSATION: Updating conversation_id: {conversation_id} with updates: {json.dumps(updates, cls=DecimalEncoder)}")
    try:
        table = dynamodb.Table(DYNAMODB_TABLE)
...
def create_intent_recognition_prompt(user_input, capabilities_context, conversation_context):
    ...
    # Safely handle user_input to prevent formatting issues
    safe_user_input = user_input.replace('"', '\\"')  # Escape double quotes
...
User's latest input: "{safe_user_input}"
...
def identify_and_process_intent(conversation_id, user_input, conversation_history):
    ...
    try:
        ...
        logger.info(f"Parsed intent: {json.dumps(parsed_intent, cls=DecimalEncoder)}")
